...
pub fn main() {
    ...
    // Postgres
    {
        ...
        // # (insert) Param: Custom
        {
            ...
            let hizat =
                bananna.field(
                    ...
                    "z437INV6D",
                    "hizat",
                    field_str().custom("integrationtests::MyString").build(),
                );
        ...
        // # Migrate - add field
        {
            ...
            let zomzom =
                bananna.field(
                    ...
                    "zPREUVAOD",
                    "zomzom",
                    field_bool().migrate_fill(Expr::LitBool(true)).build(),
                );
        ...
        // # Migrate - pre migration
        {
            ...
            v1.pre_migration(
                new_insert(&v0_two, vec![(v0_field_two.id.clone(), Expr::LitI32(7))]).build_migration(),
            );
    ...
    // Sqlite
    {
        ...
        // # Base: create table, insert, select
        {
            ...
            let bananna = v.table("zEOIWAACJ");
            let hizat = bananna.field(&mut v, "z437INV6D", "hizat", field_str().build());
            goodormning::sqlite::generate(&root.join("tests/sqlite_gen_base_insert.rs"), vec![(0usize, v)], vec![
                // Queries
                new_insert(&bananna, vec![(hizat.id.clone(), Expr::Param {
                    name: "text".into(),
                    type_: hizat.def.type_.type_.clone(),
        ...
        // # (insert) Param: i32
        {
            ...
            let bananna = v.table("zJCPRHK37");
            let hizat = bananna.field(&mut v, "z437INV6D", "hizat", field_i32().build());
            goodormning::sqlite::generate(&root.join("tests/sqlite_gen_param_i32.rs"), vec![(0usize, v)], vec![
                // Queries
                new_insert(&bananna, vec![(hizat.id.clone(), Expr::Param {
                    name: "val".into(),
                    type_: hizat.def.type_.type_.clone(),
        ...
        // # (insert) Param: Opt`<i32>`
        {
            ...
            let bananna = v.table("z8JI0I1E4");
            let hizat = bananna.field(&mut v, "z437INV6D", "hizat", field_i32().opt().build());
            goodormning::sqlite::generate(&root.join("tests/sqlite_gen_param_opt_i32.rs"), vec![(0usize, v)], vec![
                // Queries
                new_insert(&bananna, vec![(hizat.id.clone(), Expr::Param {
                    name: "val".into(),
                    type_: hizat.def.type_.type_.clone(),
        ...
        // # (insert) Param: Opt`<i32>`, null
        {
            ...
            let bananna = v.table("zT7F4746C");
            let hizat = bananna.field(&mut v, "z437INV6D", "hizat", field_i32().opt().build());
            goodormning::sqlite::generate(
                &root.join("tests/sqlite_gen_param_opt_i32_null.rs"),
                vec![(0usize, v)],
                vec![
                    // Queries
                    new_insert(
        ...
        // # (insert) Param: Custom
        {
            ...
            goodormning::sqlite::generate(&root.join("tests/sqlite_gen_param_custom.rs"), vec![(0usize, v)], vec![
                // Queries
                new_insert(&bananna, vec![(hizat.id.clone(), Expr::Param {
                    name: "val".into(),
                    type_: hizat.def.type_.type_.clone(),
        ...
        // # (insert) Param: Opt`<Custom>`
        {
            ...
            goodormning::sqlite::generate(&root.join("tests/sqlite_gen_param_opt_custom.rs"), vec![(0usize, v)], vec![
                // Queries
                new_insert(&bananna, vec![(hizat.id.clone(), Expr::Param {
                    name: "text".into(),
                    type_: hizat.def.type_.type_.clone(),
        ...
        // # Update
        {
            ...
            let bananna = v.table("zSPEZNHA8");
            let hizat = bananna.field(&mut v, "z437INV6D", "hizat", field_str().build());
            goodormning::sqlite::generate(&root.join("tests/sqlite_gen_update.rs"), vec![(0usize, v)], vec![
                // Queries
                new_insert(
                    &bananna,
                    vec![(hizat.id.clone(), Expr::LitString("yog".into()))],
        ...
        // # Delete
        {
            ...
            let bananna = v.table("zLBDEHGRB");
            let hizat = bananna.field(&mut v, "z437INV6D", "hizat", field_str().build());
            goodormning::sqlite::generate(&root.join("tests/sqlite_gen_delete.rs"), vec![(0usize, v)], vec![
                // Queries
                new_insert(
                    &bananna,
                    vec![(hizat.id.clone(), Expr::LitString("seeon".into()))],
        ...
        // # Migrate - add field
        {
            ...
            goodormning::sqlite::generate(&root.join("tests/sqlite_gen_migrate_add_field.rs"), vec![
                // Versions (previous)
                (0usize, {
                    let mut v = Version::default();
                    let bananna = v.table("zTWA93SX0");
        ...
        // # Migrate - remove field
        {
            ...
            let bananna = v.table("z1MD8L1CZ");
            let hizat = bananna.field(&mut v, "z437INV6D", "hizat", field_str().build());
            goodormning::sqlite::generate(&root.join("tests/sqlite_gen_migrate_remove_field.rs"), vec![
                // Versions (previous)
                (0usize, {
                    let mut v = Version::default();
                    let bananna = v.table("z1MD8L1CZ");
        ...
        // # Migrate - add table
        {
            ...
            let two = v.table("zHXF3YVGQ");
            let field_two = two.field(&mut v, "z156A4Q8W", "two", field_i32().build());
            goodormning::sqlite::generate(&root.join("tests/sqlite_gen_migrate_add_table.rs"), vec![
                // Versions (previous)
                (0usize, {
                    let mut v = Version::default();
                    let bananna = v.table("z4RGW742J");
        ...
        // # Migrate - remove table
        {
            ...
            let bananna = v.table("zX7CEK8JC");
            bananna.field(&mut v, "z437INV6D", "hizat", field_str().build());
            goodormning::sqlite::generate(&root.join("tests/sqlite_gen_migrate_remove_table.rs"), vec![
                // Versions (previous)
                (0usize, {
                    let mut v = Version::default();
                    let bananna = v.table("zX7CEK8JC");
        ...
        // # Migrate - pre migration
        {
            ...
            let v1_bananna = v1.table("zMI5V9F2V");
            v1_bananna.field(&mut v1, "z437INV6D", "hizat", field_str().build());
            goodormning::sqlite::generate(&root.join("tests/sqlite_gen_migrate_pre_migration.rs"), vec![
                // Versions (previous)
                (0usize, v0),
                (1usize, v1)
            ], vec![]).unwrap();
