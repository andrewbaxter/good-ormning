...
pub fn generate_migrations(output: &mut Output, versions: &Vec<(i64, Version)>) -> Result<(), Vec<String>> {
    ...
    for (version_i, version) in versions {
        ...
        fn do_migration_query(
            ...
            let mut qctx = SqliteQueryCtx::new(errs.clone(), version_i, &field_lookup);
        ...
        // Main migrations
        {
            let mut state = SqliteMigrateCtx::new(errs.clone(), version_i);
            crate::buildtime::graphmigrate::migrate(
                &mut state,
                prev_version.take().map(|s| &s.schema),
                &version.schema,
            );
...
pub fn generate_queries(
    ...
    for q in queries {
        ...
        let mut ctx = SqliteQueryCtx::new(errs.clone(), version.0, &field_lookup);
            ...
            fn convert_one_res(
                            ...
                            let x =
                                chrono::DateTime::<chrono::Utc>::from(
                                    chrono::DateTime::<chrono::FixedOffset>::parse_from_rfc3339(
                                        &x,
                                    ).map_err(|e| good_ormning::runtime::Error::Other(e.to_string()))?,
                                );
                        }
                    },
                };
                if let Some(custom) = &v.type_.custom {
                    ...
                    if v.opt {
                        ...
                        unforward = quote!{
                            #unforward let x = #ident:: from_sql(
                                x
                            ).map_err(|e| good_ormning::runtime::Error::Other(e.to_string())) ?;
                        };
                    }
        ...
        match q.res_count {
            QueryResCount::None => {
                output.data.push(quote!{
                    ...
                    good_ormning:: runtime:: Error > {
                        db.execute(#q_text, rusqlite::params![#(#args_forward,) *]) ?;
                        Ok(())
                    }
                });
            },
            QueryResCount::MaybeOne => {
                ...
                output.data.push(quote!{
                    ...
                    good_ormning:: runtime:: Error > {
                        ...
                        let mut rows = stmt.query(rusqlite::params![#(#args_forward,) *])?;
                        let r = rows.next()?;
                        if let Some(r) = r {
                    ...
                    good_ormning:: runtime:: Error > {
                        ...
                        let r =
                            rows
                                ...
                                .ok_or_else(
                                    || good_ormning::runtime::Error::Other(
                                        "Query expected to return one row but returned no rows".into(),
                                    ),
                                )?;
                        Ok(#unforward_res)
                    }
