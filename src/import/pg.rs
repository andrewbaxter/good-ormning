...
fn parse_index_columns(indexdef: &str) -> Result<Vec<String>, loga::Error> {
    ...
    let mut stmts =
        Parser::parse_sql(&dialect, indexdef)
            .map_err(|e| loga::err(format!("Failed to parse index DDL: {}", e)))?;
...
/// Read the current schema from a PostgreSQL client, returning a `Version`
/// using the existing good-ormning schema model.
pub async fn read_schema(client: &Client) -> Result<Version, loga::Error> {
    ...
    for table_name in &table_names {
        ...
        // Non-constraint indexes: use pg_index, filtering out pk/fk/unique constraint indexes.
        let idx_rows = client
            .query(
